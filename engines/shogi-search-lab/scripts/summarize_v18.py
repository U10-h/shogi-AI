#!/usr/bin/env python3
"""Summarize frozen v0.18 measurements without treating repeated roots as games."""
...
for ms in [1000,3000,5000,10000]:
 vals=[]
 for k in roots:
  a,b=lookup[k,'root',ms],lookup[k,selected,ms]
...
fallback=[]
for name in ['root','cache','guard','both']:
...
audits=read(D/'audit.json')['rows'];audit={'visits':sum(r['analysis']['stats'].get('qguard_audits',0) for r in audits),'missedAlpha':sum(r['analysis']['stats'].get('qguard_missed_alpha',0) for r in audits),'referenceNodes':sum(r['analysis']['stats'].get('qguard_audit_nodes',0) for r in audits),'roots':len(audits)}
summary={'quality':qs,'pairedQuality':paired,'timeComparisons':comparisons,'games':games,'matches':msum,'fallback':fallback,'audit':audit,'notes':['12 roots from four newly seeded teacher trajectories. Training overlap not fully ruled out.','One timing run per root/variant/budget; same pair rotates execution order.','No Elo or statistical strength claim. Four trajectory clusters too few for a stable generalization interval.','Total plies and sustained 500cp disadvantage are different observations; no certified defeat threshold.']}
