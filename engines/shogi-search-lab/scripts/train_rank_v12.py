...
def main():
    ...
    rng=np.random.default_rng(protocol['seed']);step=0;history=[];termination='epoch_limit'
    for epoch in range(1,61):
        ...
        if not np.all(np.isfinite(pred)) or np.std(pred)<1e-6:
            termination='invalid_float_projection';break
        ...
        if epoch==1 or epoch%5==0:
            if any(not np.all(np.isfinite(p)) for p in params) or max(abs(params[3]*16*SCALE))>=2**30:
                termination='invalid_integer_export';break
            ...
            # Projection followed by int8 clipping can undo the desired scale.
            # Keep the last valid checkpoint and stop before saturation propagates.
            if not (.9<=row['stdRatioTrain']<=1.1 and abs(row['meanCorrectionTrainCp'])<=1):
                row['rejected']='quantized_projection_distortion';termination=row['rejected'];break
    ...
    result={'protocol':protocol,'baseline':baseline,'affineControlValidation':metrics(head_score(x0,control),pairs,val),'selected':best,'history':history,'modelSha256':modelsha,'termination':termination,
