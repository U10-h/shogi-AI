...
s=json.loads((D/'summary.json').read_text());audit=json.loads((D/'legal-audit.json').read_text())
quality={(r['variant'],r['ms']):r for r in s['quality']};sel=json.loads((D/'selection.json').read_text())
rows=json.loads((D/'overhead.json').read_text())['rows'];old=new=0
for i in range(8):
 rs=[r for r in rows if r['id']==i];old+=statistics.median(r['old']['elapsed_ms'] for r in rs);new+=statistics.median(r['off']['elapsed_ms'] for r in rs)
out=['# v0.18：長時間探索の効果と、静止探索の再利用実験','',
...
'## 実装したもの','',
'1. **qcache**：静止探索で読み終えた値・読み筋を再利用。正確値／上界／下界を区別し、同じ完全手順で到達した局面だけに使う。中断した節点は保存しない。','2. **qguard**：駒損と推定される駒取りを選別。王手回避・王手・成り・取り返し・玉の着手・両玉から2マス以内の駒取りを保護する。SEEによるヒューリスティックであり、見落としのない枝刈りではない。','3. 両機能の併用も比較。NNUEと学習済みの根方策の重みは変更していない。深さ・候補数を固定せず、費用による選択的探索を維持した。','',
...
for ms in [1000,3000,5000,10000]:out.append(f"| {ms/1000:g}秒 | {quality['root',ms]['meanGap']:.2f}cp | {quality['cache',ms]['meanGap']:.2f}cp |")
out+=['','基準方式の対応比較では、1→3秒で **4改善・8同等・0悪化**、3→5秒で全12同等、5→10秒で **1改善・11同等・0悪化** だった。1秒から10秒への改善は複数局面で起きたが、5秒から10秒の改善は根 `1-56` の341cp→0cpに集中した。長く考える利益は時間に比例せず、特定の戦術を読み切れる段階で現れた。','',
...
for g in s['games']:
 out.append(f"| {'基準' if g['variant']=='root' else '再利用'} | {g['ourMs']/1000:g}秒 | {'先手' if g['side']=='black' else '後手'} | {g['totalPlies']} | {'負け' if g['outcome']=='loss' else g['outcome']} | {g['firstSustained500Ply'] if g['firstSustained500Ply'] is not None else '未観測'} |")
out+=['','500cp欄は、相手の探索評価で500cp以上不利が3回連続して観測された、その最初の時点。確定した敗着ではない。手数が伸びた対局も抵抗力の観測として残し、不利になった後に長引いた場合と区別する。各方式・時間は2局だけなので、勝率や時間配分の優劣は断定しない。','',
'再利用版の5秒・後手は133手で、基準の同条件91手より長く戦った。500cp不利の継続開始も50手→72手へ遅れており、この1局は抵抗力が改善した観測として評価できる。一方、先手5秒では110手→86手に短くなり、効果は混在した。各条件1局ずつのため、この好結果だけで方式を採用しない。','', '## 静止探索の問題は解決したか','',
...
f"qguardの監査は既知3局面・合計{s['audit']['visits']:,}訪問でα改善の見落とし{s['audit']['missedAlpha']}件。ただし確認探索の合計も{s['audit']['referenceNodes']:,}ノードで、今回の監査対象はいずれも子の1ノードで終わった。深い犠牲手まで安全だったという証拠にはならない。監査対象は相関した訪問であり独立標本ではない。",'',
'## 正しさ・再現性','',
...
f'- 同一ノードの24対応比較では、局面ごとの時間中央値の合計比（新ビルド無効／旧ビルド）は{new/old:.3f}。この環境では新ビルド全体の速度低下は観測しなかった。qcacheを有効にした効果とは別の測定。',
f"- 独立ルール実装tsshogiで、{audit['gameCount']}局・{audit['gameMoves']:,}着手、{audit['searches']:,}探索出力、対局・自作探索のPV延べ{audit['pvMoves']:,}手、別途教師候補{audit['teacherPVs']}本を検査。全KIFの読み戻しが一致。終局は両実装で確認。教師の生成進行256手も合法。", 
...
(R/'REPORT-v0.18.md').write_text('\n'.join(out)+'\n')
print('REPORT-v0.18.md written')
