...
class Worker {
    ...
    Value qsearch(int a,int beta,int ply,int left) {
        ...
        if(deferred)++r.stats["q_deferred_nodes"];
            ...
            generate();
            if(moves.empty()&&(!o.direct_qmoves||checked||!b.has_legal_move())){++r.base.terminals;return {-mate+ply,{}};}
        }
        ...
        if(!checked) {
            if(left<=0){++r.base.leaves;if(deferred)++r.stats["q_avoided_generations"];return best;}
            if(stand>=beta){++r.stats["qstand_cutoffs"];if(deferred)++r.stats["q_avoided_generations"];return best;}
            a=std::max(a,stand);
