...
// Exact lexical order of legal USI move strings, without string construction.
...
public:
    ...
    std::vector<Move> tactical_moves() const; // Captures + every promotion; all evasions in check.
    bool has_legal_move() const;
    bool has_legal_king_move() const;
    void remember();
...
struct Iteration {
    int depth = 0, score = 0;
    uint64_t nodes = 0, pv_priority_nodes = 0;
    double elapsed_ms = 0;
    std::vector<Move> pv;
};
struct Result {
