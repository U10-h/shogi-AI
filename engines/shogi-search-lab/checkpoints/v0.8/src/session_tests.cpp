...
int session_selftest() {
    ...
    for (int policy = 0; policy < 4; ++policy) for (auto& sfen : positions) {
        TreeSession session(sfen);
        Board reference(sfen);
        for (int depth = 1; depth <= 3; ++depth) {
            Options o; o.depth = depth;
            o.top5_screen = policy == 1 || policy == 3;
            o.reply_probe = policy >= 2;
    ...
    // Abort during the extra probe work, then resume from the committed ranking.
    TreeSession probing;
    Options po; po.depth = 1; po.reply_probe = true;
    const auto p1 = probing.analyze(po);
    po.depth = 3; po.max_nodes = 4;
    const auto pa = probing.analyze(po);
    require(!pa.complete && pa.probe_nodes == 3 && pa.nodes == 4, "probe consumes shared budget");
    same_candidates(p1.view, pa.view);
    require(pa.view.sfen == p1.view.sfen, "probe abort restores board");
    po.max_nodes = 2000000;
    const auto pr = probing.analyze(po);
    TreeSession pfresh; po.reply_probe = false;
    same_candidates(pr.view, pfresh.analyze(po).view);
    require(pr.probe_nodes == 30 && pr.root_exclusions > 0, "probe reused across iterative depths and screening active");
    // Flat evaluations stress the inclusive/exclusive fifth-place threshold.
    const std::string flat = "4k4/9/9/9/9/9/9/9/4K4 b - 1";
    TreeSession ffull(flat), fscreen(flat), fprobe(flat);
    Options fo; fo.depth = 4; fo.top5_screen = false;
    const auto exact_flat = ffull.analyze(fo);
    fo.top5_screen = true;
    same_candidates(exact_flat.view, fscreen.analyze(fo).view);
    fo.reply_probe = true;
    same_candidates(exact_flat.view, fprobe.analyze(fo).view);
