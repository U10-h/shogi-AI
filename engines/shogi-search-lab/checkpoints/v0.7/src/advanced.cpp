...
class Worker {
    ...
    std::vector<Move> order(std::vector<Move> moves,int ply,Move preferred,Move previous) {
        struct Item {Move move; int64_t priority; std::string text;};
        ...
        for(Move m:moves) {
            ...
            items.push_back({m,score,usi(m)});
        }
        std::sort(items.begin(),items.end(),[](const Item&a,const Item&b){return a.priority!=b.priority?a.priority>b.priority:a.text<b.text;});
        moves.clear();for(auto& i:items)moves.push_back(i.move);return moves;
    }
    Value qsearch(int a,int beta,int ply,int left) {
        tick();++r.stats["qnodes"];
        if(!null_level)if(auto rep=b.repetition_score(ply))return {*rep,{}};
        ...
        const int stand=eval(b); Value best{checked?-infinity:stand,{}};
        if(!checked) {
        ...
        for(Move m:moves) {
            ...
            Value c;{PlayedMove move(b,m);c=qsearch(-beta,-a,ply+1,left-1);}
            if(-c.score>best.score){best={-c.score,{m}};best.pv.insert(best.pv.end(),c.pv.begin(),c.pv.end());}
            a=std::max(a,best.score);if(a>=beta){++r.base.cutoffs;break;}
        }
        return best;
    }
    ...
    Value visit(int d,int a,int beta,int ply,uint64_t path,int ext,bool allow_null,Move prev) {
        if(d<=0&&f.qsearch)return qsearch(a,beta,ply,o.qdepth);
        tick();
    ...
    Value rps(int budget,int a,int beta,int ply,Move prev) {
        if(budget<=0&&f.qsearch)return qsearch(a,beta,ply,o.qdepth);
...
std::string advanced_json(const AdvancedResult& r,const AdvancedOptions& o) {
    ...
    std::ostringstream out;out<<base<<",\"engine\":\"research-v0.7\",\"evaluation\":"<<quote(o.evaluation)<<",\"evaluation_model\":"<<quote(o.evaluation_model)<<",\"driver\":"<<quote(o.driver)<<",\"selective\":"<<(r.selective?"true":"false")
