...
Evaluator::Evaluator(const std::string& mode,const std::string& model):mode_(mode){
    if(mode!="material"&&mode!="positional"&&mode!="learned")throw std::invalid_argument("Eval must be material|positional|learned");
    if(mode!="learned"&&!model.empty())throw std::invalid_argument("--eval-model requires --eval learned");
