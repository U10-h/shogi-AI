...
root=Path('.')
paths=['README.md','THIRD_PARTY.md','LICENSE','package.json','.gitignore','.github','.openai','dist','docs','scripts','tests','vendor']
with ZipFile('dist/shogi-ai-source.zip','w',ZIP_DEFLATED) as archive:
 for item in paths:
  p=Path(item)
  for f in ([p] if p.is_file() else sorted(p.rglob('*'))):
   if f.is_file() and f.name!='shogi-ai-source.zip': archive.write(f,'shogi-AI/'+str(f))
`]);
