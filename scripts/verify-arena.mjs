...
assert.equal(d.status,'finished');assert.equal(d.settings.timeMs,3000);assert.equal(d.games.length,6);
const positionKey=s=>s.split(' ').slice(0,3).join(' ');
const summary={games:[],moves:0,pvLines:0,pvMoves:0,sourceHashes:true,passed:false};
for(const [path,hash]of Object.entries(d.batches.v04.lab.sourceFilesSha256))assert.equal(createHash('sha256').update(readFileSync('engines/shogi-search-lab/'+path)).digest('hex'),hash);
for(const g of d.games){
 const r=recordAt(START,[]);assert.equal(g.status,'finished');
 for(const m of g.moves){
  assert.equal(positionKey(m.beforeSfen),positionKey(r.position.sfen));assert.equal(m.side,r.position.color);assert.equal(m.engine,m.side===g.labSide?'lab':'yaneuraou');assert(m.wallMs>=0&&m.wallMs<5000);
  ...
  if(m.engine==='lab'){assert.equal(m.analysis.candidates[0].move,m.usi);assert(m.analysis.candidates.length>=1&&m.analysis.candidates.length<=5);assert(m.analysis.candidates.every(c=>c.depth===m.analysis.depth));if(g.labVersion!=='0.5')assert.equal(m.analysis.stopReason,'time_limit');else assert.equal(m.analysis.candidates.length,1);}
  assert(r.append(move));assert.equal(positionKey(m.afterSfen),positionKey(r.position.sfen));summary.moves++;
 }
