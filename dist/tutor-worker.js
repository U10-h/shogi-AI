...
import {TutorGenerationQueue} from './tutor-generation.js';
/* Optional, downloaded only after the user chooses to enable local conversation. */
let engine;
const queue=new TutorGenerationQueue();
self.onmessage=async({data})=>{
  if(data.type==='interrupt'){queue.interrupt();engine?.interruptGenerate();return;}
  try{
    if(data.type==='load'){
    ...
    if(data.type==='answer'){
      ...
      const result=await queue.run(()=>engine.chat.completions.create({messages,temperature:TUTOR_TUNING.temperature,max_tokens:TUTOR_TUNING.maxTokens,response_format:{type:'json_object'},stream:false}));
