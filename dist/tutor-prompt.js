...
export const TUTOR_TUNING=Object.freeze({round:6,temperature:0.15,maxTokens:650});
export function tutorMessages(data){
  ...
  return [{role:'system',content:system+'\n'+termInstructions(data.question,data.evidence)+'\n'+example+' 引用idは、説明した手順のkind=lineの根拠を必ず含める。指し手は根拠の表記をそのまま使い、「同銀」などへ省略しない。矢印の順番と何手目かを変えず、別の枝を一本の手順に混ぜない。比較するときは両方の枝を引用する。駒の収支をエンジンの評価値として言い換えない。例文を事実として引用しない。'},{role:'user',content:JSON.stringify({EVIDENCE:data.evidence,LEARNER_FORECAST:data.forecast||{},history:data.history||[],question:data.question})}];
}
